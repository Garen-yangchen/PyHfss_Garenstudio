...
######################################################################
    ...
    def getDesignvariables(self):
        Designvariables = self.oDesign.GetVariables()
        return Designvariables
    
    def getProjectvariables(self):
        Projectvariables = self.oProject.GetVariables()
...
######################################################################   
    ...
    def createEllipse(self, XCenter,YCenter,ZCenter,MajRadius,Ratio,Whichaxis,Name):
        self.oEditor.CreateEllipse(
                                ["NAME:EllipseParameters", "IsCovered:=", True, "XCenter:=", XCenter, "YCenter:=", YCenter, "ZCenter:=", ZCenter,
		                         "MajRadius:=", MajRadius, "Ratio:=", Ratio, "WhichAxis:=", Whichaxis, "NumSegments:=", "0"], 
	                            ["NAME:Attributes", "Name:=", Name, "Flags:=", "", "Color:=", "(143 175 143)", "Transparency:=", 0,
		                         "PartCoordinateSystem:=", "Global", "UDMId:=", "", "MaterialValue:=", "\"vacuum\"", "SurfaceMaterialValue:=", "\"\"", 
                                 "SolveInside:=", True, "IsMaterialEditable:=", True, "UseMaterialAppearance:=", False, "IsLightweight:=", False])	

    def createPolyline(self,Name, X1, Y1, Z1, X2, Y2, Z2):                                                            
        self.oEditor.CreatePolyline(
        	                   ["NAME:PolylineParameters", "IsPolylineCovered:=", True, "IsPolylineClosed:=", False,
       		                   ["NAME:PolylinePoints",["NAME:PLPoint", "X:=", X1, "Y:=", Y1, "Z:=", Z1],
      		            	   ["NAME:PLPoint", "X:=", X2, "Y:=", Y2, "Z:=", Z2]],
        	                   ...
        	                   ["NAME:Attributes", "Name:=", Name, "Flags:=", "", "Color:=", "(143 175 143)", "Transparency:=", 0, "PartCoordinateSystem:=", "Global",
        		                "UDMId:=", "", "MaterialValue:=", "\"vacuum\"", "SurfaceMaterialValue:=", "\"\"", "SolveInside:=", True, "IsMaterialEditable:=", True, "UseMaterialAppearance:=", False, "IsLightweight:=", False])
...
######################################################################   
    ...
    def check(self):
        Flag=self.oDesign.ValidateDesign()
        return Flag      # 0 means the validation failed, 1 means passed
...
######################################################################     
    ...
    def thickenSheet(self, Face, Thickness, Bothsides):
        if Bothsides=="T":
                Flag= True
        else:
                Flag= False         
        self.oEditor.ThickenSheet(["NAME:Selections", "Selections:=", Face, "NewPartsModelFlag:=", "Model"], 
	                       ["NAME:SheetThickenParameters", "Thickness:=", Thickness, "BothSides:=", Flag])        
...
#                                   		                           "ShowReporterFilter:=", False, "ReporterFilter:=", [False,False], "UseScanAngles:=", True, "Phi:=", Scan_phi, "Theta:=", Scan_theta,
    ...
    def assignLumpedport(self, Portname, Object, Startx,Starty,Startz,Endx,Endy,Endz):

        self.oModule = self.oDesign.GetModule("BoundarySetup")
        self.oModule.AssignLumpedPort(
        	                      ["NAME:"+Portname, "Objects:=", [Object], "DoDeembed:=", False, "RenormalizeAllTerminals:=", True,
        		                  ["NAME:Modes",["NAME:Mode1", "ModeNum:=", 1, "UseIntLine:=", True,
        				          ["NAME:IntLine", "Start:=", [str(Startx), str(Starty),str(Startz)], "End:=", [str(Endx), str(Endy), str(Endz)]], "AlignmentGroup:=", 0, "CharImp:=", "Zpi", "RenormImp:=", "50ohm"]],"ShowReporterFilter:=", False, "ReporterFilter:=", [True], "Impedance:=", "50ohm"])
...
######################################################################  
    ...
    def createRectangulargaincurve(self, Reportname, Setupname, Phi, Theta, Result_items):
        self.oModule = self.oDesign.GetModule("ReportSetup")
        self.oModule.CreateReport(Reportname, "Far Fields", "Rectangular Plot", Setupname+": Sweep", 
    	["Context:=", "Infinite Sphere1"], 
    	["Freq:=", ["All"], "Phi:=", Phi, "Theta:=", Theta], 
    	["X Component:=", "Freq", "Y Component:=", Result_items], [])
        
 
    def createRectangularfarfieldpreport(self, Reportname,Setupname,Theta, Phi,Freq):
        if Theta=='All':
            theta='All'
            phi=str(Phi)+'deg'
            X_Component='Theta'
...
#            reportname=Reportname+' Theta= '+str(Theta)+' deg'
    ...
    def deleteAllsolutions(self):
        self.oDesign.DeleteFullVariation("All", False)
...
######################################################################  
    def exportField(self, Fieldtype, Savepath, Savefilename, Xmin, Xmax, Xstep, Ymin, Ymax, Ystep, Zmin, Zmax, Zstep):
        ...
        self.oModule.ExportOnGrid(Savepath+"\\"+Savefilename, 
